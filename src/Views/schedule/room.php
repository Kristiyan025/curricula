<h2><i class="bi bi-door-open"></i> Разписание на зала</h2>

<div class="row mb-4">
    <div class="col-md-6">
        <form method="GET" action="/schedule/room" class="d-flex">
            <select class="form-select me-2" name="room_id" onchange="this.form.submit()">
                <option value="">-- Изберете зала --</option>
                <?php foreach ($rooms ?? [] as $r): ?>
                    <option value="<?= $r['id'] ?>" <?= ($selectedRoom ?? '') == $r['id'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars($r['room_number']) ?> 
                        (<?= $r['capacity'] ?> места, 
                        <?= $r['room_type'] === 'LECTURE_HALL' ? 'лекционна' : ($r['room_type'] === 'COMPUTER_LAB' ? 'компютърна' : 'семинарна') ?>)
                    </option>
                <?php endforeach; ?>
            </select>
        </form>
    ...
    <div class="col-md-6 text-end">
        ...
        <a href="/schedule/lecturer" class="btn btn-outline-secondary">По преподавател</a>
    </div>
</div>

<?php if (empty($selectedRoom)): ?>
    <div class="alert alert-info">
        Моля, изберете зала от списъка.
    </div>
<?php elseif (empty($schedule)): ?>
    <div class="alert alert-warning">
        Няма генерирано разписание за избраната зала.
    </div>
<?php else: ?>
    <div class="card">
        <div class="card-header card-header-fmi">
            <i class="bi bi-door-open"></i> 
            Зала <?= htmlspecialchars($roomInfo['room_number']) ?>
            <span class="badge bg-secondary ms-2"><?= $roomInfo['capacity'] ?> места</span>
        </div>
                                    ...
                                    <td class="<?= $slot ? 'slot-' . strtolower($slot['slot_type']) : '' ?>">
                                        <?php if ($slot): ?>
                                            ...
                                            <small class="text-muted">
                                                ...
                                                <?= htmlspecialchars($slot['lecturer_name'] ?? '') ?>
                                            </small>
                                        <?php else: ?>
    ...
    <div class="mt-3">
        ...
        <span class="badge slot-exercise p-2 me-2">Упражнение</span>
        <span class="badge bg-light text-success border p-2"><i class="bi bi-check-circle"></i> Свободна</span>
    </div>
<?php endif; ?>
